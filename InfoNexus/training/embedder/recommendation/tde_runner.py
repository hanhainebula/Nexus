...
from InfoNexus.abc.training.embedder import AbsEmbedderRunner
from InfoNexus.modules.optimizer import get_lr_scheduler, get_optimizer
...
class TDERetrieverRunner(RetrieverRunner):
    ...
    def load_model(self) -> BaseRetriever:
        ...
        model = TDEModel(model) # wrap 
        model, tde_configs_dict, tde_feature_names, tde_settings = self._prepare_model(model) # prepare 
        return model, tde_configs_dict, tde_feature_names, tde_settings
    
    def load_trainer(self, tde_configs_dict, tde_feature_names, tde_settings) -> TDERetrieverTrainer:    
