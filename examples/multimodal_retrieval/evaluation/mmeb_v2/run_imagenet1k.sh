...
RUN_ROOT=${RUN_ROOT:-$NEXUS_ROOT/outputs/mmeb_v2/imagenet1k_$(date +%Y%m%d_%H%M%S)}
NPROC=${NPROC:-8}
BATCH_SIZE=${BATCH_SIZE:-32}
MASTER_PORT=${MASTER_PORT:-29741}
TORCHRUN=${TORCHRUN:-torchrun}
CONFIG=${CONFIG:-$SCRIPT_DIR/configs/imagenet1k_only.yaml}
OUT_SUBDIR=${OUT_SUBDIR:-imagenet1k}

export PYTHONPATH="$NEXUS_ROOT:${PYTHONPATH:-}"
mkdir -p "$RUN_ROOT/logs" "$RUN_ROOT/generated_media_cache" "$RUN_ROOT/outputs"
cd "$NEXUS_ROOT"

echo "[MMEB v2] task=imagenet1k"
...
"$TORCHRUN" \
  ...
  2>&1 | tee "$RUN_ROOT/logs/imagenet1k.log"
