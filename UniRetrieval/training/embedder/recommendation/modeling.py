...
class MLPRetriever(AbsEmbedderModel):
    def __init__(
            self, 
            data_config: DataAttr4Model,
            model_config: Union[Dict, str],
            model_type : str='retriever', 
            *args, **kwargs
        ):
        # from BaseModel
        super(MLPRetriever, self).__init__(*args, **kwargs)
        ...
        # from self.init_modules()
        self.item_encoder = ItemEncoder(self.data_config, self.model_config)
        self.query_encoder = QueryEncoder(self.data_config, self.model_config,self.item_encoder)
        self.score_function = InnerProductScorer()
        self.negative_sampler = UniformSampler(num_items=self.data_config.num_items)
    ...
    # Belows are all copied from BaseRetriever
