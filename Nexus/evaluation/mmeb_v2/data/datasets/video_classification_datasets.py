...
LOCAL_VIDEO_JSONL = {
    "HMDB51": "hmdb51.jsonl",
    "UCF101": "ucf101.jsonl",
    "Breakfast": "breakfast.jsonl",
    "Kinetics-700": "k700.jsonl",
    "SmthSmthV2": "ssv2-actiontemplate.jsonl",
}


def _normalize_instruction(instruction, fallback):
    instruction = (instruction or fallback or "Classify the video into the correct category.").strip()
    if instruction.endswith(":"):
        instruction = instruction[:-1] + "."
    return instruction


def _resolve_local_jsonl_path(dataset_name, data_args, kwargs):
    data_path = kwargs.get("data_path")
    if data_path and os.path.exists(data_path):
        return data_path

    data_basedir = getattr(data_args, "data_basedir", None)
    filename = LOCAL_VIDEO_JSONL.get(dataset_name)
    if data_basedir and filename:
        candidate = os.path.join(data_basedir, "video-tasks", "data", filename)
...
def data_prepare(batch_dict, **kwargs):
    ...
    default_instruction = DATASET_INSTRUCTION.get(dataset_name, "Classify the video into the correct category.")
    ...
    for video_id, label, original_video_path in zip(
        batch_dict['video_id'], batch_dict['pos_text'], original_video_paths
    ):
...
def load_video_class_dataset(model_args, data_args, **kwargs):
    dataset_name = kwargs['dataset_name']
    local_jsonl_path = _resolve_local_jsonl_path(dataset_name, data_args, kwargs)
