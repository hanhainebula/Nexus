...
def main():
    ...
    base_model_path = "/zhdd/home/lynx/wuzeyu/model/Qwen2-VL-2B-Instruct" 
    lora_path = model_args.model_name_or_path  # 你通过 bash 脚本传入的 checkpoint 路径

    # DDP-safe model loading
    # Step 1: 只有 rank 0 先加载模型
    if rank == 0:
        print_master(f"[rank=0] Loading the BASE model from: {base_model_path}...")
        ...
        print_master(f"[rank=0] Loading LoRA adapter from: {lora_path}...")
    ...
    # Step 2: 其他进程等待 rank 0 加载完毕
    if torch.distributed.is_initialized():
        torch.distributed.barrier()
    
    # Step 3: 非主进程从缓存加载
    if rank != 0:
        print_rank(f"Loading the BASE model and LoRA from cache...")
