...
def prepare_task_paths(task_config, data_args):
    raw_paths = {key: task_config.get(key) for key in ["image_root", "video_root", "frame_root", "clip_root", "data_path"]}

    if data_args.data_basedir is not None:
        for key, value in raw_paths.items():
            if value:
                task_config[key] = os.path.join(data_args.data_basedir, value)

    if (
        data_args.generated_media_basedir not in [None, ""]
        and task_config.get("dataset_parser") in {"vidore", "visrag"}
        and raw_paths.get("image_root")
    ):
        task_config["image_root"] = os.path.join(data_args.generated_media_basedir, raw_paths["image_root"])

    _cap_video_frame_fields(task_config, data_args.max_video_frames)

    return task_config


def _cap_video_frame_fields(task_config, max_video_frames):
    if max_video_frames in [None, ""]:
        return
    max_video_frames = int(max_video_frames)
    if max_video_frames <= 0:
        return
    for key in ["num_frames", "max_frames_saved", "num_video_frames", "max_video_frames_saved"]:
        if key in task_config and task_config[key] not in [None, "", "None"]:
            task_config[key] = min(int(task_config[key]), max_video_frames)
    for key in ["num_clip_frames", "max_clip_frames_saved"]:
        if key in task_config and task_config[key] not in [None, "", "None"]:
            task_config[key] = min(int(task_config[key]), max_video_frames)
...
def encode_embeddings(
    ...
    for batch_inputs, dataset_info in progress_bar:
        with torch.autocast(enabled=torch.cuda.is_available(), dtype=torch.bfloat16, device_type=autocast_device):
    ...
    if dist.is_initialized():
        # NCCL 只能 gather CUDA tensor，确保 embedding 在当前 rank 的 GPU 上
        device = local_embeds_tensor.device
        local_embeds_tensor = local_embeds_tensor.to(device, non_blocking=True).contiguous()

        gathered_embeds = [
            torch.empty_like(local_embeds_tensor, device=device)
...
def main():
    ...
    print_rank(f"Process Identity -> Rank: {rank}, Local Rank: {local_rank} on {device_name}")
    ...
    if rank == 0:
        ...
        model = MMEBEmbeddingModel.load(
            ...
            max_frames=data_args.max_video_frames,
    ...
    for dataset_idx, (dataset_name, task_config) in enumerate(dataset_configs.items()):
        ...
        do_query = data_args.force_recompute or not os.path.exists(query_embed_path) or not os.path.exists(dataset_info_path)
        do_cand = data_args.force_recompute or not os.path.exists(cand_embed_path)

        if do_query or do_cand:
            task_config = prepare_task_paths(task_config, data_args)
        ...
        if rank == 0:
            ...
            need_compute = True
            if not data_args.force_recompute and os.path.exists(score_path) and os.path.exists(pred_path):
                try:
            ...
            if need_compute:
                ...
                if rank_against_all_candidates:
                    ...
                    for qid, (ranked_idx, gt_info) in tqdm(
                        ...
                        pred_dicts.append({
                            "prediction": [cand_keys[int(i)] for i in ranked_idx],  # Save complete ranking
                            "label": rel_docids,
                    ...
                    for qid, (qry_vec, gt_info) in tqdm(
                        ...
                        pred_dicts.append({
                            "prediction": [cand_names[int(i)] for i in ranked_idx],
                            "label": rel_docids,
                            "rel_scores": rel_scores,
                        })
